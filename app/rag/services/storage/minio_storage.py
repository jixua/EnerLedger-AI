...
class MinioStorage(BaseObjectStorage):
    ...
    def upload_from_path(
        ...
        """用 boto3 transfer manager 分块上传文件，避免大文件完整驻留内存。"""
    ...
    def remove_object(self, bucket: str, object_key: str) -> bool:
        """幂等删除单个对象。

        S3 ``DeleteObject`` 对不存在的 key 也返回成功，因此返回值只表示
        请求已成功执行，不声称删除前对象一定存在。
        """
    ...
    def _delete_batch(self, bucket: str, batch: list[dict[str, str]]) -> int:
        ...
        if errors:
            ...
            raise OSError(
                ...
                f"示例 key={sample.get('Key')} code={sample.get('Code')} "
                f"msg={sample.get('Message')}"
            )
