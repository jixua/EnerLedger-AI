...
from app.rag.models.db_models import UsageLogDB
...
class UsageLogService:
    ...
    async def get_user_usage(
        ...
        if start_date:
            stmt = stmt.where(
                UsageLogDB.created_at >= datetime.combine(start_date, datetime.min.time())
            )
        if end_date:
            stmt = stmt.where(
                UsageLogDB.created_at <= datetime.combine(end_date, datetime.max.time())
            )
