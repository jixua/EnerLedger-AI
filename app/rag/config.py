...
class Settings(BaseSettings):
    ...
    # v2 采用 Ark 2048 维联合 dense+sparse，与旧 1024 维 collection 隔离。
    CHUNK_INDEX_COLLECTION_NAME: str = "tolink_rag_chunks_v2"
    ...
    DENSE_VECTOR_DIMENSION: int = 2048
    ...
    def validate_document_queue_settings(self) -> "Settings":
        ...
        try:
            retry_delays = [
                int(item.strip())
                for item in self.DOCUMENT_QUEUE_RETRY_DELAYS_SECONDS.split(",")
                if item.strip()
            ]
        except ValueError as exc:
            raise ValueError(
                "DOCUMENT_QUEUE_RETRY_DELAYS_SECONDS must be comma-separated integers"
            ) from exc
