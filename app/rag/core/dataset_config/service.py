...
按 ``(user_id, dataset_id)`` 只读当前 ``dataset`` 表中的模型绑定，并把未落库的切块、
增强、PDF 与召回参数从 Settings 组装为 :class:`DatasetParseConfigBundle`。

**职责边界**：执行面纯只读。找不到数据集时返回绑定为空的内存 bundle（不写库）；
DB 读取失败向上抛出，不得伪装成“没有绑定”。数据集行由当前项目控制面维护。
...
def _load_enhancement_config(value) -> EnhancementConfig:
    ...
    当前最小数据集投影固定返回 ``{}``，表示不启用表格、图片或标题层级增强；
    非空对象仍按迁入契约叠加 Settings，便于以后扩展而无需改执行接口。
    """
    overrides = value
    if overrides == {}:
