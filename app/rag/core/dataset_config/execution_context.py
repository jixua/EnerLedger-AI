...
from app.rag.core.llm.provider_lifecycle import (
    aclose_dataset_execution_contexts,
    aclose_resolved_models,
)
from app.rag.core.llm.runtime_repository import RuntimeConfigRepository
from app.rag.core.llm.user_model_resolver import ResolvedModel, aresolve_model
...
class DatasetExecutionContextLoader:
    ...
    async def load(
        ...
        try:
            ...
            await aclose_resolved_models(
                resolved for _capability, resolved in resolved_by_id.values()
            )
            raise
    ...
    async def load_many(
        ...
        try:
            ...
            await aclose_dataset_execution_contexts(result.values())
            raise
