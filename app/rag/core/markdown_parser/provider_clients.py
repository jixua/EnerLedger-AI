...
from app.rag.core.prompts.markdown_enhancement import (
...
if TYPE_CHECKING:
    from app.rag.core.llm.interfaces import BaseProvider
...
async def abuild_table_client(resolved: "ResolvedModel", *, user_id: int) -> "ProviderTableClient":
...
class ProviderTableClient(TableClient):
    ...
    def describe_tables(self, tables, source_file=None):
        raise RuntimeError(
            "ProviderTableClient only supports async usage. Please call `adescribe_tables`."
        )
...
class ProviderVisionClient(VisionClient):
    ...
    def describe_images(self, image_urls, source_file=None, image_bytes_by_url=None):
        raise RuntimeError(
            "ProviderVisionClient only supports async usage. Please call `adescribe_images`."
        )
    ...
    async def adescribe_images(
        ...
        return {image_url: description for image_url, description, _usage in triples if description}
