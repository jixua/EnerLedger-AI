...
from app.rag.core.mq.interfaces import IMQSender, IMQReceiver
from app.rag.core.mq.exceptions import (
    MQConnectionError,
    MQSendError,
    MQConsumeError,
)
from app.rag.core.mq.retry import (
    DLQPublisher,
    DispatchOutcome,
    RetryPolicy,
    dispatch_with_retry,
...
class KafkaSender(IMQSender):
    ...
    async def _ensure_producer(self) -> None:
        ...
        try:
            ...
            logger.info(f"[Kafka Producer] 连接成功: {self._bootstrap_servers}")
        except ImportError:
        ...
        *,
        ...
        try:
            ...
            if headers or delay_ms:
                ...
                kafka_headers = [(k, v.encode("utf-8")) for k, v in raw_headers.items()]
        ...
        try:
            ...
            for i, msg in enumerate(messages):
                ...
                if metadata is None:
                    ...
                    logger.debug(f"[Kafka] batch 溢出，回退到单条发送: msg_index={i}")
            # 发送 batch 中剩余的消息
            ...
            logger.debug(f"[Kafka] 批量发送完成: topic={topic}, count={len(messages)}")
        except MQSendError:
            raise
        except Exception as e:
            raise MQSendError(
...
class KafkaReceiver(IMQReceiver):
    ...
    async def subscribe(
        ...
        *,
        ...
        logger.info(f"[Kafka Consumer] 注册订阅: topic={topic}, group={group_id}")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            offset_reset = "earliest" if primary["from_beginning"] else self._auto_offset_reset
            ...
            self._running = True
            logger.info(f"[Kafka Consumer] 启动成功: topics={topics}, group={primary['group_id']}")
        ...
        try:
            async for msg in self._consumer:
                ...
                metadata = {
                    ...
                    "key": msg_key,
                    "headers": (
                        {k: v.decode("utf-8") for k, v in msg.headers} if msg.headers else {}
                    ),
                }
