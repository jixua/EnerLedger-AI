...
from app.rag.observability.logging import sanitize_url_for_log, truncate_log_value
    ...
    async def send(
        ...
        *,
        ...
        try:
            ...
            logger.debug(f"[RabbitMQ] 消息已发送 -> routing_key={routing_key}")
        except Exception as e:
            raise MQSendError(
...
class RabbitMQReceiver(IMQReceiver):
    ...
    async def subscribe(
        ...
        *,
        ...
        logger.info(f"[RabbitMQ Consumer] 注册订阅: queue={topic}, tag={group_id}")
            ...
            for sub in self._subscriptions:
                ...
                async def _on_message(
                    ...
                    if outcome in (DispatchOutcome.OK, DispatchOutcome.DLQ_PUBLISHED):
                        ...
                        logger.error(f"[RabbitMQ] DLT 投递失败 nack-requeue: queue={_queue_name}")
