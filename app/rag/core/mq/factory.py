...
from app.rag.core.mq.exceptions import MQConfigError
from app.rag.core.mq.interfaces import IMQReceiver, IMQSender, MQVendorType
from app.rag.core.mq.retry import DLQPublisher, RetryPolicy
...
class MQFactory:
    ...
    def _register_defaults(self) -> None:
        """注册内置的厂商适配器"""
        from app.rag.core.mq.vendors.kafka.kafka_adapter import KafkaReceiver, KafkaSender
        from app.rag.core.mq.vendors.rabbitmq_adapter import RabbitMQReceiver, RabbitMQSender
    ...
    def _resolve_config(self) -> Dict[str, Any]:
        ...
        if vendor == MQVendorType.KAFKA:
            ...
            raise MQConfigError(
                f"不支持的 MQ 厂商: {vendor}，可选: {list(self._sender_registry.keys())}",
                vendor=vendor,
            )
    ...
    def get_receiver(self, **overrides: Any) -> IMQReceiver:
        ...
        # 业务回调异常时走 app.rag.core.mq.retry.dispatch_with_retry，而不是裸 try/except。
    ...
    def get_dlq_publisher(self) -> DLQPublisher:
        ...
        签名见 :data:`app.rag.core.mq.retry.DLQPublisher`：
