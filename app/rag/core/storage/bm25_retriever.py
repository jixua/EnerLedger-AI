...
from app.rag.core.pipeline.recall.protocols import SOURCE_BM25
from app.rag.core.storage.bm25_exceptions import is_transient_bm25_error
from app.rag.core.storage.bm25_models import Bm25ChunkHit, Bm25RecallRequest
from app.rag.utils.logger import logger
...
class Bm25Retriever:
        ...
        *,
        ...
        for dataset_id in dataset_ids:
            ...
            for doc_id in doc_iter:
                ...
                hits = await self._recall_with_retry(request)
                for hit in hits:
                    dataset_hits.append(
                        RetrieverHit(
    ...
    async def _recall_with_retry(self, request: Bm25RecallRequest) -> list[Bm25ChunkHit]:
        """Retry only connection-level transient reads for both recall entry paths."""

        for attempt in range(BM25_READ_MAX_ATTEMPTS):
            try:
                return await self._backend.recall_topk_chunks(request)
            except Exception as exc:
                if attempt + 1 >= BM25_READ_MAX_ATTEMPTS or not is_transient_bm25_error(exc):
                    raise
                await asyncio.sleep(BM25_READ_RETRY_BASE_SECONDS * (2**attempt))
