...
from app.rag.core.pipeline.recall.protocols import SOURCE_SPARSE
...
class SparseRetriever:
    ...
    async def recall(
        ...
        *,
        ...
        from app.rag.core.pipeline.recall.exceptions import RecallFatalError
        from app.rag.core.storage.vector.exceptions import (
            VectorRetrievalUserConfigMissingError,
        )

        accumulated: list[RetrieverHit] = []
        for dataset_id in dataset_ids:
            context = dataset_contexts.get(dataset_id) if dataset_contexts is not None else None
