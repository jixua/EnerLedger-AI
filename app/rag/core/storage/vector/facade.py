...
from app.rag.core.splitter.models import Chunk
...
if TYPE_CHECKING:
    from app.rag.core.encoding.sparse import SparseVectorService
    from app.rag.core.splitter.embedding_pipeline import ChunkEmbeddingPipeline
    from app.rag.core.storage.vector.compensation_pipeline import (
        VectorStorageCompensationPipeline,
    )
    from app.rag.core.storage.vector.management_pipeline import (
        VectorStorageManagementPipeline,
    )
    from app.rag.core.storage.vector.pipeline import VectorStoragePipeline
...
class VectorStorageFacade:
    ...
    def __init__(
        self,
        *,
        storage_service: VectorStoragePipeline | None = None,
        management_service: VectorStorageManagementPipeline | None = None,
        compensation_service: VectorStorageCompensationPipeline | None = None,
    ...
    async def search_sparse_chunks(
        ...
        **完全只读**：不改 MySQL ``document_chunk``、不调 Qdrant
        ...
        from app.rag.core.encoding.sparse import (
            SparseVectorConfigurationError,
            SparseVectorError,
        )
        from app.rag.core.storage.qdrant.exceptions import (
            QdrantStoreError,
            QdrantVectorStorageConfigurationError,
        )
        from app.rag.core.storage.qdrant.models import SparseQueryVectorSpec
    ...
    async def search_dense_chunks(
        ...
        *,
        ...
        ``ChunkEmbeddingPipeline`` 向量化（按 ``dataset.dense_embedding_config_id``
        ...
        **完全只读**：不改 MySQL ``document_chunk``、不调 Qdrant
        ...
        Returns:
            ...
            - 每个 hit 含 ``chunk_id`` / ``doc_id`` / ``set_id`` / ``score`` /
              ...
              后由上层查 ``document_chunk`` 回填真值，并通过 Document ``READY`` 门禁
              排除未完成或失败文档。
        ...
        from app.rag.core.storage.qdrant.models import DenseQueryVectorSpec
        ...
        if effective_threshold < 0 or effective_threshold > 1.0:
            raise ValueError(
                f"score_threshold must be in [0, 1] (cosine bound), got {effective_threshold!r}"
            )
