...
from app.rag.application.recall_errors import CODE_SCOPE_FORBIDDEN, RecallApiError
from app.rag.core.storage.chunks.constants import CHUNK_LIFECYCLE_ACTIVE
from app.rag.core.storage.document_visibility import (
...
from app.rag.core.wiki.models import (
    WIKI_NODE_CHUNK_REF,
    WIKI_NODE_HEADING,
    EffectiveWikiScope,
...
from app.rag.models.parse_task import DocumentParsePipeline, DocumentParseTask
from app.rag.models.wiki_tree import WikiTreeNodeDB
    ...
    async def replace_document_tree(
        ...
        for chunk_ref in tree_draft.chunk_refs:
            if (
                ...
                raise ValueError(
                    f"chunk reference points to unknown heading: {chunk_ref.parent_heading_key}"
                )
        ...
        for heading_level in sorted(headings_by_level):
            ...
            if missing_keys:
                raise RuntimeError(
                    f"failed to resolve inserted wiki heading ids: {sorted(missing_keys)}"
                )
