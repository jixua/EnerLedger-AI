...
class DerivedElementChunkBuilder:
    ...
    def _positive_page_number(value: Any) -> int | None:
        """Parse one positive page number from element metadata."""
        try:
            page = int(value)
    ...
    @classmethod
    def _page_metadata(cls, element: MarkdownElement) -> dict[str, Any]:
        """Copy and normalize page provenance for an image/table derived chunk."""
        source = element.metadata
        page_number = cls._positive_page_number(source.get("page_number"))
        explicit_pages = source.get("page_numbers")
        pages: set[int] = set()
        if page_number is not None:
            pages.add(page_number)
        if isinstance(explicit_pages, (list, tuple, set, frozenset)):
            pages.update(
                page
                for value in explicit_pages
                if (page := cls._positive_page_number(value)) is not None
            )

        start_page = cls._positive_page_number(source.get("start_page"))
        end_page = cls._positive_page_number(source.get("end_page"))
        range_values = [page for page in (start_page, end_page) if page is not None]
        all_bounds = [*pages, *range_values]
        if not all_bounds:
            return {}
        ...
        metadata["start_page"] = min(all_bounds)
        metadata["end_page"] = max(all_bounds)
        return metadata
