...
class CandidateBoundaryChunker:
    ...
    def _merge_trailing_heading_chunk(self, bundles: list[_ChunkBundle]) -> None:
        ...
        page_numbers = sorted(
            {
                int(page)
                for view in previous_chunk.element_views
                if (page := view.metadata.get("page_number")) is not None
            }
        )
        if page_numbers:
            previous_chunk.metadata["page_numbers"] = page_numbers
            previous_chunk.metadata["start_page"] = page_numbers[0]
            previous_chunk.metadata["end_page"] = page_numbers[-1]
        ...
        page_numbers = sorted(
            {
                ...
                if (page := element.metadata.get("page_number")) is not None
            }
        )
        if page_numbers:
