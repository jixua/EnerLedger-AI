...
from app.rag.core.markdown_parser import ElementType
from app.rag.core.mq.exceptions import RetriableError
...
class _CohesionScorer:
    ...
    async def score(self, atoms: list[_Atom]) -> _GapScores:
        ...
        if scoreable:
            ...
            vec_by_idx = {
                idx: vec for (idx, _), vec in zip(scoreable, vectors, strict=True)
            }
            ...
            try:
                ...
                if transient:
                    raise _Stage2EmbeddingError(
                        f"Stage 2 cohesion embedding failed after {EMBED_RETRY_LIMIT} retries"
                    ) from exc
                raise
...
class _SegmentPacker:
    ...
    def pack(self, atoms: list[_Atom], scores: _GapScores) -> list[list[_Atom]]:
        ...
        非评分 protected（代码/公式）接到已有非空 segment 上时放宽到 hard_max，
        避免其单独成无文本块。
        """
        ...
        page_numbers = sorted(
            {
                int(page)
                for atom in segment
                if (page := atom.metadata.get("page_number")) is not None
            }
        )
        if page_numbers:
