        ...
        try:
            async with client.stream("POST", url, json=payload, headers=headers) as response:
                if response.status_code >= 400:
                    ...
                    if response.status_code == 401:
                        raise AuthenticationError(
                            message="Invalid API Key", provider_type="anthropic"
                        )
                    if response.status_code == 429:
                        raise RateLimitError(
                            message="Rate limit exceeded", provider_type="anthropic"
                        )
                    raise ProviderConnectionError(
                        message=f"Anthropic API error: {response.status_code}",
                        provider_type="anthropic",
        ...
        async for chunk in self._client.stream_messages(
            ...
            **kwargs,
            ...
            if ctype == "message_start":
                ...
                completion_tokens = (
                    usage.get("output_tokens", completion_tokens) or completion_tokens
                )
