...
from app.rag.core.markdown_parser.text_formatter import TextFormatter
from app.rag.core.parser.factory import ParserFactory

if TYPE_CHECKING:
    from app.rag.core.dataset_config import DatasetExecutionContext, EnhancementConfig
...
class ParseTaskService:
    ...
    async def _aprocess_impl(
        ...
        **parser_kwargs,
        ...
        enhanced_parse_result = await orchestrator.aenhance_parse_result(
            ...
            enhancement_config=enhancement_config,
            enhancement_chat=(
                execution_context.enhancement_chat if execution_context is not None else None
            ),
            enhancement_vision=(
                execution_context.enhancement_vision if execution_context is not None else None
            ),
        )
        ...
        if enhancement_config is not None:
            from app.rag.core.markdown_parser.heading_hierarchy import HeadingHierarchyConfig
    ...
    @staticmethod
    def _apply_pdf_page_markers(parse_result) -> int:
        """把 OpenDataLoader 页分隔标记转换为元素页码，并从正文元素中移除标记。

        OpenDataLoader 2.5 会在每页开头写入 ``ODL_PAGE`` HTML 注释。这里按元素顺序
        继承页码，确保后续切块能够聚合真实页范围；标记本身不进入检索正文。
        """
