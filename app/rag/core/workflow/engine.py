...
from app.rag.core.workflow.constants import FailurePhase, NodeStatus, RunStatus
...
from app.rag.core.workflow.store import NodeRunRecord, RunRecord, WorkflowStore
...
class WorkflowEngine:
    ...
    async def _run_main_loop(
        ...
        while True:
            ...
            if task is not None:
                await task
            required_failed = await self._consume_completion(event, store, run_id, ctx, node_states)
    ...
    def _resolve_resume_plan(
        ...
        restore = {
            node_key for node_key in skip if set(nodes_by_key[node_key].provides) & needed_products
        }
