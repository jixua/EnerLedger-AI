...
class ParseDeleteRepository:
    ...
    async def delete_parse_rows_by_doc_id(
        ...
        log_rows = await db.execute(
            delete(DocumentParsedLog).where(DocumentParsedLog.document_original_file_id == doc_id)
        )
        file_rows = await db.execute(
            delete(DocumentParseTask).where(DocumentParseTask.document_original_file_id == doc_id)
        )
        return {
