...
class RawMarkdownAssetLoader:
    ...
    async def load_batch(self, uris: Iterable[str], payload) -> dict[str, tuple[bytes, str]]:
        unique = list(dict.fromkeys(uris))
        results = await asyncio.gather(*(self.load_one(uri, payload) for uri in unique))
        return {
            uri: loaded for uri, loaded in zip(unique, results, strict=True) if loaded is not None
        }
