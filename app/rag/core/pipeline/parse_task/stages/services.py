...
if TYPE_CHECKING:
    from app.rag.core.dataset_config import (
        ChunkingConfig,
...
from app.rag.core.markdown_parser import (
    ImageDescriber,
    MarkdownParser,
    ParseResult,
    abuild_vision_client,
)
from app.rag.core.mq.messages.parse_task import ParseTaskPayload
from app.rag.core.parse_task_service import ParseTaskService
from app.rag.core.preprocessor.models import FilePostIndexPlan
from app.rag.core.splitter import create_chunking_engine
...
from app.rag.core.splitter.models import Chunk, ChunkingResult
from app.rag.core.storage.bm25_backend import build_indexing_pipeline
...
from app.rag.core.storage.chunks.repository import ChunkRepository
from app.rag.core.storage.index_mutation_guard import (
    MutationGuardProtocol,
    NoopIndexMutationGuard,
)
from app.rag.core.storage.index_mutation_models import IndexBranch
...
from app.rag.core.storage.vector.models import ChunkIndexingResult
from app.rag.core.storage.wiki_tree import WikiTreeRepository
from app.rag.core.wiki import HeadingTreeBuilder
...
class StageServices:
            ...
            if loaded:
                if describer is None:
                    ...
                    if client is None:
                        ...
                        from app.rag.core.markdown_parser import build_default_vision_client
        ...
        try:
            ...
            try:
                ...
                logger.bind(
                    ...
                    "[StageServices] rollback after BM25 guarded failure also failed: task_id={}",
                    task_id,
                )
            logger.bind(
        ...
        *,
        ...
        try:
            ...
            logger.bind(
                ...
                "[StageServices] BM25 写入异常，将清理可能半成品: doc_id={} error_type={} error={}",
                meta.doc_id,
                type(exc).__name__,
                truncate_log_value(exc),
            )
    ...
    async def run_sparse_vectorizing(
        ...
        """
        from app.rag.core.storage.vector.sparse_indexing import SparseIndexingPipeline
        ...
        *,
        ...
        try:
            ...
            logger.bind(
                ...
                "[IndexWriteCleanupAlert] event=cleanup_failed task_id={} branch={} chunk_count={}",
                task_id,
                branch.value,
                len(chunk_ids),
            )
    ...
    def _get_preprocessor(self) -> PreprocessorProtocol:
        ...
        try:
            from app.rag.core.preprocessor.service import Preprocessor
        except Exception as exc:
