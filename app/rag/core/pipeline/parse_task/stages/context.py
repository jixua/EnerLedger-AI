...
class StageContext:
    ...
    def success_result(self) -> ParsePipelineResult:
        ...
        return ParsePipelineResult(
            ...
            page_count=(
                self.parse_result["metadata"].get("pages_or_length", 0) if self.parse_result else 0
            ),
            vector_indexing_completed=True,
            failed_chunk_ids=self.vector_result.failed_chunk_ids if self.vector_result else [],
        )

    def failure_result(self, outcome: "StageOutcome", *, failed_stage: str) -> ParsePipelineResult:
