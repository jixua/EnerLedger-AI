...
class SparseVectorizingStage(Stage):
    ...
    async def run(self, ctx: StageContext) -> StageOutcome:
        ...
        from app.rag.core.storage.vector.sparse_indexing import SparseIndexingError

        try:
            await self._services.run_sparse_vectorizing(ctx.payload, ctx.db, ctx.execution_context)
