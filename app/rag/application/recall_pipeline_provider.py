...
from app.rag.core.pipeline.ltr.candidate_routing import (
...
from app.rag.core.pipeline.recall import RecallPipeline, RecallPipelineConfig, RecallRequest
from app.rag.core.pipeline.recall.document_readiness import MySqlDocumentReadinessGate
from app.rag.core.pipeline.recall.protocols import (
    SOURCE_BM25,
    SOURCE_DENSE,
    SOURCE_SPARSE,
    Retriever,
...
@lru_cache(maxsize=1)
def _get_vector_recall_facade():
    """让 Dense/Sparse 召回共用一个 Qdrant 客户端。"""
...
def _build_sparse_retriever() -> Retriever:
    return SparseRetriever(
        backend=_get_vector_recall_facade(),
