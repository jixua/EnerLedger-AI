...
function statusMeta(document) {
  ...
  if (normalized === 'READY') return { label: '已完成', modifier: 'ready', icon: CheckCircle2 };
...
export function TasksPage() {
  ...
  const counts = useMemo(() => {
    const next = { ALL: allDocuments.length, QUEUED: 0, PROCESSING: 0, READY: 0, FAILED: 0 };
  ...
  return (
    <div className="page page--tasks">
      ...
      <section className="task-metrics" aria-label="任务状态摘要">
        ...
        <article className="task-metric task-metric--ready"><CheckCircle2 size={17} /><div><strong>{counts.READY}</strong><span>已完成</span></div></article>
                ...
                {filteredDocuments.map((document) => {
                  ...
                  return (
                    <tr key={`${datasetId}:${documentId(document)}`}>
                      ...
                      <td>
                        {normalizedStatus(document.status) === 'FAILED' ? (
                          <span className="table-error" title={document.error_message || ''}>{document.error_message || '解析或索引失败'}</span>
                        ) : normalizedStatus(document.status) === 'READY' ? (
                          <span>{document.chunk_count ?? 0} 个片段 · {document.page_count ?? '-'} 页</span>
                        ) : (
                          <span className="muted-copy">{normalizedStatus(document.status) === 'QUEUED' ? '等待处理' : `第 ${document.attempt_count || 1} 次尝试`}</span>
                        )}
                      </td>
                      ...
                      <td>
                        <div className="task-row-actions">
                          <Link className="text-link" to={`/datasets/${datasetId}`}>查看数据集</Link>
                          {normalizedStatus(document.status) === 'FAILED' ? <button type="button" className="button button--tiny" onClick={() => runDocumentAction(document, actions.retryDocument, '文档已重新加入队列。')} disabled={busyDocumentId === documentId(document)}><RefreshCw className={busyDocumentId === documentId(document) ? 'spin' : ''} size={13} /> 重试</button> : null}
                          {normalizedStatus(document.status) === 'READY' ? <button type="button" className="button button--tiny" onClick={() => runDocumentAction(document, actions.reparseDocument, '文档已加入重新解析队列。')} disabled={busyDocumentId === documentId(document)}><RefreshCw className={busyDocumentId === documentId(document) ? 'spin' : ''} size={13} /> 重新解析</button> : null}
                        </div>
                      </td>
                    </tr>
                  );
