...
- The backend has no conversation persistence API. Never present local messages as server-side conversation history; label them as current-page-only when this distinction matters.
...
- Document upload is asynchronous: successful uploads enter a MySQL-backed durable parsing queue and normally return `QUEUED`. Do not describe upload as synchronous parsing or claim that there is a separate task table.
