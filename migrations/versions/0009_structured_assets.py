...
Revises: 0008_document_folder_hierarchy
...
down_revision: str | None = "0008_document_folder_hierarchy"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("user_id", "dataset_id", "asset_code", name="uk_structured_asset_dataset_code"),
    )
    op.create_index("idx_structured_asset_dataset", "structured_asset", ["user_id", "dataset_id", "updated_at"])
    op.create_table(
    ...
    op.create_index("idx_structured_version_asset_state", "structured_asset_version", ["asset_id", "state", "created_at"])
    op.create_index("idx_structured_version_edition", "structured_asset_version", ["asset_id", "edition_year", "created_at"])
    op.create_table(
    ...
    op.create_index("idx_structured_table_version", "structured_table", ["version_id", "table_code"])
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), server_default=sa.func.current_timestamp(), nullable=False),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("user_id", "dataset_id", "term", "canonical_value", name="uk_structured_term_alias"),
    )
    op.create_index("idx_structured_term_lookup", "structured_term_alias", ["user_id", "dataset_id", "term"])
    op.create_table(
    ...
    op.create_index("idx_structured_query_user_created", "structured_query_audit", ["user_id", "created_at"])
