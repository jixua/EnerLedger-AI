...
本项目的技术方案是把“入库质量门禁—结构化分片—三路混合召回—证据约束生成”连成一条可检查的数据链，并在其上接入 Pi Agent 的受控工具编排和报告生成工作流。其目标是让问答和报告中的事实能够回到当前有效版本的原始文档和具体片段，并在某一路检索或重排不可用时明确降级。当前代码实现了这些工程机制；能碳领域的端到端准确率尚需建立人工金标后测量。
...
对话中可以 `@` 一份已处于 `READY` 状态的知识库文档。若本轮是在提问，FastAPI 将检索范围收窄到该文档，智能体仍可按目录和章节工具继续阅读；若请求生成报告，该文档成为冻结版本的报告来源。用户也可以直传材料，服务端提取文本并返回 `material_id`；这类材料可用于本轮问答或报告，但不建立知识库文档和三路索引，也不会进入普通召回。来源材料和版式模板可分别来自知识库文档或直传文件；对话入口先判定问答或报告意图，再识别来源与模板角色，报告类型不明确时请用户确认，不静默选择多份来源中的一份。

报告任务是与即时 RAG 回答衔接的异步工作流：FastAPI 冻结来源、报告模板和模型版本，写入任务并经 outbox/RabbitMQ 投递给独立 `report-worker`，由 Pi report-agent 处理。当前模板注册表提供 R1–R7 七类报告。报告 Agent 必须先加载公共及类型专属 Skill，按稳定顺序分页读完冻结来源；可检索参考知识，但报告事实须写入 Evidence Ledger，模板字段须记录 Field Ledger。注册公式由确定性计算工具执行；缺失或冲突的阻塞字段进入 `NEEDS_INPUT` 向用户提问。报告正文先保存为 ReportIR，经服务端校验后才提交，再由同一结构渲染在线预览、Markdown、DOCX 和 HTML。直传材料在报告任务内部形成专用冻结分片并接受覆盖校验，它与知识库的检索分片及三路索引相互独立。
