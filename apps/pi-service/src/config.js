...
export function loadConfig(env = process.env) {
  ...
  const backendToken = env.REPORT_AGENT_INTERNAL_TOKEN ?? "";
  ...
  if (serviceToken && backendToken && serviceToken === backendToken) {
    throw new Error("Inbound and outbound service tokens must be different");
  }
  ...
  const allowedModelHosts = String(env.REPORT_MODEL_ALLOWED_HOSTS ?? "")
    ...
    .filter(Boolean);
  if (production && !allowedModelHosts.length) {
    throw new Error("REPORT_MODEL_ALLOWED_HOSTS is required in production");
  }
  return {
    ...
    toolTimeoutMs: positiveNumber(env.REPORT_AGENT_TOOL_TIMEOUT_SECONDS, 30, "REPORT_AGENT_TOOL_TIMEOUT_SECONDS") * 1000,
    runTimeoutMs: positiveNumber(env.REPORT_AGENT_RUN_TIMEOUT_SECONDS, 600, "REPORT_AGENT_RUN_TIMEOUT_SECONDS") * 1000,
    maxToolCalls: positiveNumber(env.REPORT_AGENT_MAX_TOOL_CALLS, 80, "REPORT_AGENT_MAX_TOOL_CALLS"),
    allowedModelHosts,
